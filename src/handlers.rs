...
use crate::models::{
    ...
    response::{
        ...
        OpenAICompatibleResponse, StreamEvent, Usage,
    },
};
...
fn extract_api_tokens(
    ...
    // 首先尝试从环境变量获取API tokens
    if let (Ok(deepseek), Ok(anthropic)) = (
                ...
                Ok(event) => match event {
                    ...
                    crate::clients::anthropic::StreamEvent::MessageDelta { usage, .. } => {
                        ...
                        if let Some(usage) = usage {
                            ...
                            let anthropic_cost = calculate_anthropic_cost(
                                "claude-3-7-sonnet-20250219", // Default model
                            ...
                            let (deepseek_usage, deepseek_cost) = if let Some(usage) = deepseek_usage.as_ref() {
                                let cost = calculate_deepseek_cost(
                                    usage.input_tokens,
                                    usage.output_tokens,
                                    usage.output_details.reasoning,
                                    usage.input_details.cached,
                                    &config,
                                );
                                ...
                                (DeepSeekUsage {
                                    ...
                                    reasoning_tokens: usage.output_details.reasoning,
                                    cached_input_tokens: usage.input_details.cached,
